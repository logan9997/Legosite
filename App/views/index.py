...
def index(request):
    ...
    new_items = FORMATTER.format_item_info(DB.get_new_items(
    )[:10], item_group='_new_items', graph_data=[graph_metric])

    last_week = dt.today() - timedelta(days=7)
    last_week = last_week.strftime('%Y-%m-%d')

    for popular_item in popular_items:
        popular_item['change'] = round(DB.get_weekly_item_metric_change(
            popular_item['item_id'], last_week, graph_metric), 2)

    username = User.objects.filter(
        user_id=user_id).values_list('username', flat=True)
    ...
    if user_id == -1 or len(DB.get_user_items(user_id, 'portfolio')) == 0:
        ...
        context['trending'] = FORMATTER.format_item_info(DB.get_biggest_trends(
            graph_metric, limit=10), metric_trends=[graph_metric], graph_data=[graph_metric], user_id=user_id)
    else:
