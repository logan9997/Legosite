    ...
    if request.method == 'POST':
        ...
        if request.POST.get('form-type') == 'change-password-form':
            ...
            if form.is_valid():
                ...
                rules: list[dict] = [
                    {len(User.objects.filter(user_id=user_id, password=old_password))
                     > 0: "'Old password' is incorrect"},
                ...
                if all([all(rule) for rule in rules]):
                    User.objects.filter(password=old_password, user_id=user_id).update(
                        password=new_password)
                else:
                    # pass an error message to context, based on what condition was not satisfied
                    context['change_password_error_message'] = get_change_password_error_message(
                        rules)
