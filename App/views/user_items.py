...
from config.config import (ALL_METRICS, METRIC_INPUT_STEPS,
...
def user_items(request, view, user_id):
    ...
    items = FORMATTER.format_item_info(
        items, owned_quantity_new=8, owned_quantity_used=9, graph_data=[graph_metric], user_id=user_id)
    ...
    filter_results = FILTER_OUT.process_filters(
        request, items,  [theme['theme_path'] for theme in themes])
    ...
    items = items[(current_page - 1) * USER_ITEMS_ITEMS_PER_PAGE:
                  int(current_page) * USER_ITEMS_ITEMS_PER_PAGE]
...
def view_POST(request, view):
    ...
    if '?item=' in request.META.get('HTTP_REFERER'):
        request.session['graph_metric'] = request.POST.get(
            'graph-metric', 'avg_price')
        request.session['graph_options'] = GENERAL.sort_dropdown_options(
            get_graph_options(), request.POST.get('graph-metric', 'avg_price'))
    ...
    portfolio_items = FORMATTER.format_item_info(
    ...
    if request.POST.get('form-type') == 'remove-or-add-portfolio-item':
        ...
        if form.is_valid():
            ...
            if (item_id, condition) in [(_item['item_id'], _item['condition']) for _item in portfolio_items]:
                DB.update_portfolio_item_quantity(
                    user_id, item_id, condition, quantity)
            else:
                DB.add_to_user_items(item_id, user_id, view,
...
def portfolio(request, item_id=None):
    ...
    context['total_bought_price'] = Portfolio.objects.filter(user_id=user_id).aggregate(
        total_bought_for=Sum('bought_for', default=0))['total_bought_for']
    context['total_sold_price'] = Portfolio.objects.filter(user_id=user_id).aggregate(
        ...
        if form.is_valid():
            ...
            for _ in range(quantity):
                DB.add_to_user_items(
                    user_id, item_id, view, date_added, condition=condition, bought_for=bought_for)
    else:
...
def get_sub_themes(user_id: int, parent_themes: list[str], themes: list[dict], indent: int, view: str, metric: str) -> list[str]:
    ...
    for theme in parent_themes:
        ...
        sub_themes = [
            theme_path for theme_path in sub_themes if theme_path.count('~') == indent]
        themes.append({
            'theme_path': theme[0],
            'count': theme[1],
