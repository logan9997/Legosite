...
def search(request, theme_path='all'):
    ...
    if theme_path == 'all':
        sub_themes = [{"sub_theme": theme[0], "img_path":f"App/sets/{theme[1]}.png"}
        ...
        theme_items = FORMATTER.format_item_info(
            theme_items, graph_data=[graph_metric], user_id=user_id)
        ...
        sub_themes = [{"sub_theme": theme[0].split("~")[sub_theme_indent], "img_path":f"App/sets/{theme[1]}.png"}
                      for theme in DB.get_sub_theme_set(theme_path.replace("/", "~"), sub_theme_indent)]

    total_theme_items = Theme.objects.filter(
        theme_path=theme_path.replace("/", "~"), item__item_type="M").count()
